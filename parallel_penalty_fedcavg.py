...
def fedcavg_partial(dat_gen = None, res_dir = 'res', num_of_cls = 10, num_of_locs = 100, seed = 1, \
        nu = 1e-4, rho = 1e-8, H_max_iter = 100, W_max_iter = 100, is_diminish_epoch = False, \
        is_diminish_step = False, num_of_active_nodes = 10, select_method = 'uniform_replace_partial', \
        DEBUG = False):
    ...
    res_manager.add_cost_value('sncp_cost', get_sncp_cost(data_mat, n_factor, W_init, H_init, nu, 0, rho, device = device))
    ...
    while True:
        ...
        if num_of_active_nodes == num_of_locs:
        ...
        elif num_of_active_nodes < num_of_locs:
            if select_method == 'uniform_replace_all':
            ...
            elif select_method == 'uniform_replace_partial':
                selected_nodes = np.random.choice(num_of_locs, num_of_active_nodes, True, prob)
                for node_id in selected_nodes:
                    node = local_nodes[node_id]
                    node.receive_W(W_pre)
            ...
            raise ValueError('Error: the number of active nodes is bigger than that of clients!!')
        ...
        elif is_diminish_step:
            W_iters = int(W_max_iter)
            Hessian = 2 * torch.mm(H_cur, torch.transpose(H_cur, 0, 1)) / data_num
            egenvals, _= torch.eig(Hessian)
            c = 5 * sncp_iter * torch.max(egenvals).item()
        else:
            W_iters = int(W_max_iter)
            Hessian = 2 * torch.mm(H_cur, torch.transpose(H_cur, 0, 1)) / data_num
            egenvals, _= torch.eig(Hessian)
        ...
        #W_cur = np.minimum(max_val, np.maximum(0, W_cur))
...
def fedcavg(dat_gen = None, res_dir = 'res', num_of_cls = 10, num_of_locs = 100, seed = 1, \
    ...
    while True:
        ...
        if is_diminish_epoch:
            ...
            Hessian = 2 * torch.mm(H_cur, torch.transpose(H_cur, 0, 1)) / data_num
            egenvals, _= torch.eig(Hessian)
            c = 2 * torch.max(egenvals).item()

        for node_id in range(num_of_locs):
            node = local_nodes[node_id]
            W_tmp = node.update_W_by_bcd(W_iters, c)
            #W_cur += (W_tmp - W_pre) * H_tmp.shape[1] / H_cur.shape[1]
            W_cur = 2 * torch.add(W_cur, W_tmp - W_pre, alpha = node.N_p / data_num)
...
def main():
    ...
    nu = 1e-10
    rho = 1e-8
    is_diminish_epoch = False
    is_diminish_step = False
    select_method = 'uniform_replace_all'

    #for dat_name in ['snr-3', 'mnist_bal', 'mnist_unbal']:
    for dat_name in ['tcga11']:
        #for par_m in ['similarity']:
        for par_m in ['uniform']:
        #for par_m in ['ubal_noniid#2']:
            if dat_name.startswith('snr'):
                is_real, num_of_cls, num_features, num_data, tol = False, 20, 2000, 10000, 2e-5
            elif dat_name.startswith('mnist'):
                is_real, num_of_cls, num_features, num_data, tol = True, 10, 784, 10000, 8e-5
            elif dat_name.startswith('tcga'):
                if dat_name == 'tcga5':
                    is_real, num_of_cls, num_features, num_data, tol = True, 33, 2000, 11135, 1e-4
                elif dat_name == 'tcga6':
                    ...
                    is_real, num_of_cls, num_features, num_data, tol = True, 20, 5000, 5314, 2e-5
            ...
            #for H_iter in [1, 10, 50, 100]: # the number of epochs for W update
            for s_num in [100]:
                for H_iter in [1]:
                    #for W_iter in [300]: # the number of epochs for H update
                    for W_iter in [1]:
                        W_step = ''
                        if is_diminish_epoch: W_step = 'D' + str(W_iter)
                        elif is_diminish_step: W_step = str(W_iter) + 'Ds'
                        else: W_step = str(W_iter)
                        # get the result directory where the result is stored
                        res_dir = os.path.join(parent_dir, 'result', 'fedcavg_partial_new', 'ds_gamma5_w1', dat_name,  \
                                par_m + '#' + str(num_of_pars), select_method + '#' + str(s_num), \
                                'H#' + str(H_iter) + '_W#' + W_step + '_rho#' + str(rho) + '_nu#' + str(nu)) 
                        ...
                        #for seed_num in [1]:
                            ...
                            fedcavg_partial(dat_gen = data_gen, res_dir = seed_dir, num_of_cls = num_of_cls, \
                                    num_of_locs = num_of_pars, seed = seed_num, nu = nu, rho = rho, H_max_iter = H_iter, \
                                    W_max_iter = W_iter, num_of_active_nodes = s_num, is_diminish_epoch = is_diminish_epoch, \
                                    is_diminish_step = is_diminish_step, select_method = select_method, DEBUG = True)
                            '''
                            fedcavg(dat_gen = data_gen, res_dir = seed_dir, num_of_cls = num_of_cls, \
                                    num_of_locs = num_of_pars, seed = seed_num, nu = nu, rho = rho, H_max_iter = H_iter, \
                                    W_max_iter = W_iter, is_diminish_epoch = is_diminish_epoch, DEBUG = True)
                            ''' 
