...
class Node(object):
    ...
    Attributes:
        local_data (tensor): the data owned by a local site
        data_scale (float): normalized factor used to normalize the dataset
        #W (tensor): the primal variable W in SNCP
        H (tensor): the primal variable H in SNCP
    ...
    def __init__(self, data, data_num = 10000, rank = 4, init_W = None, init_H = None, \
            init_rho = 1e-8, nu = 1e-4, mul = 0, para = 1e-6, max_val = 100, device = 'cpu'):
        ''' __init__ method to initialize all the fields

        Args:
            data (tensor): the input data array
        ...
        self.HHT = torch.mm(self.H, torch.transpose(self.H, 0, 1))
        self.XHT = torch.mm(self.local_data, torch.transpose(self.H, 0, 1))
    ...
    def send_W_grad_diff(self):
        ...
        '''
        HHT = torch.mm(self.H, torch.transpose(self.H, 0, 1))
        XHT = torch.mm(self.local_data, torch.transpose(self.H, 0 , 1))

        diff_HHT, diff_XHT = torch.add(HHT, -self.HHT), torch.add(XHT, -self.XHT)
        self.HHT, self.XHT = HHT, XHT
    ...
    def update_penalty(self, beta, max_val = 1000):
        ''' This function is used to increase the penalty parameter rho
...
def get_onmf_cost(data, n_factor, W, H, nu = 1e-4, mul = 0):
    ...
    Args:
        ...
        W (tensor): the factor W
        H (tensor): the factor H
...
def fedcgds_with_partial_nodes(dat_gen = None, res_dir = 'res', num_of_cls = 10, \
        num_of_locs = 100, seed = 1, nu = 1e-4, rho = 1e-8, tol = 2e-3, selection = 'uniform', \
    ...
    full_W_grad = torch.zeros(M, num_of_cls)
    ...
    HHT = torch.mm(H_pre, torch.transpose(H_pre, 0, 1))
    XHT = torch.mm(data_mat, torch.transpose(H_pre, 0, 1))
    W_max = max_val * torch.ones_like(W_pre, device = device, dtype = torch.float64)
    W_zeros = torch.zeros_like(W_pre, device = device, dtype = torch.float64)

    p_max_iter = 200 # the max number of iterations for each subproblem
    ...
    while True:
        ...
        if DEBUG:
            ...
            print ('Iter(' + str(sncp_iter) + '), after update H, cost = ' + str(sncp_cost))
        ...
        """
        egenvals, _ = torch.eig(HHT)
        full_max_eval = torch.max(egenvals).item()
        t = 0.501 * full_max_eval
        #t = 0.51 * full_max_eval
        #print ('t: ' + str(t * 2 / data_num))
        W_tmp = W_pre.clone().detach()
        #W_max = max_val * torch.ones_like(W_pre, device = device, dtype = torch.float64)
        #W_zeros = torch.zeros_like(W_pre, device = device, dtype = torch.float64)
        #print ('W_pre ' + str(W_pre))
        for j in range(W_max_iter):
            full_W_grad = torch.add(torch.mm(W_tmp, HHT), XHT, alpha = -1)
            W_cur = torch.min(torch.max(torch.add(W_tmp, full_W_grad, alpha = -1 / t), W_zeros), W_max)
            W_tmp = W_cur
        print ('W converge #' + str(j))
        #print (W_cur)
        del W_tmp

        #W_change = torch.norm(W_cur - W_pre) / torch.norm(W_pre)

        if DEBUG:
            #onmf_cost = master_node.get_onmf_cost()
            sncp_cost = get_sncp_cost(data_mat, n_factor, W_cur, H_cur, nu, mul, rho, device)
            print ('Iter(' + str(sncp_iter) + '), after update W, cost = ' + str(sncp_cost))
        ...
        #print ('W_change: ' + str(W_change) + ' cost_c: ' + str(sncp_cost_nr) + ' ortho: ' + str(res_manager.peek_H_norm_residual()))
        print (' cost_c: ' + str(sncp_cost_nr) + ' ortho: ' + str(res_manager.peek_H_norm_residual()))
        print ('ACC: ' + str(acc))

        if sncp_cost_nr < 1e-8 or sncp_iter > 1000: # converges
            print ('Converges------')

            # draw a figure to show the clustering result
            dat_path = os.path.join(res_dir, 'res.pdf')
            print (dat_path)
            H_cpu = np.asmatrix(H_cur.cpu().data.numpy())
            print (H_cpu.shape)
            cls_assign = np.asarray(np.argmax(H_cpu, axis = 0))[0, :] # get the cluster assignments
            print (cls_assign)
            #dat_gen.visualize_data(partition_idx = cls_assign, dat_path = dat_path)

            del W_pre, W_cur, H_pre, H_cur, full_W_grad, data_mat, HHT, XHT, H_tmp, W_max, W_zeros
            for node_id in range(num_of_locs):
                node = local_nodes[node_id]
                del node.I_ha, node.all_1_ts, node.H_zeros, node.HHT, node.XHT
            torch.cuda.empty_cache()
            res_manager.write_to_csv()  # store the generated results to csv files

            break
        else: 
            # if not converge, increase rho and send w when
            # subproblem is solved to certain accuracy or
            # the subproblem is solved with p_max_iter iterations
            # then update the W in local nodes
            for node_id in range(num_of_locs):
                node = local_nodes[node_id]
                ...
                if sncp_cost_nr < tol or sncp_iter % p_max_iter == 0:
                    if node_id == 0:
                        ...
                        temp_iter = sncp_iter
                    node.update_penalty(beta_rho, n_factor / data_num)
...
def main():
    ...
    #for dat_name in ['mnist_unbal', 'tdt3', 'tcga11']:
        #for par_m in ['ubal_noniid', 'bal_noniid', 'weighted', 'uniform']:
        #for par_m in ['ubal_noniid#2', 'bal_noniid#2']:
        #for par_m in ['ubal_noniid#2', 'bal_noniid#2']:
        #for par_m in ['uniform', 'similarity']:
        for par_m in ['similarity']:
            if dat_name.startswith('snr'):
                ...
                if dat_name == 'tcga5':
                    ...
                    is_real, num_of_cls, num_features, num_data, tol = True, 20, 5000, 5314, 5e-5
                else:
            ...
            #for s_num in [50, 70, 100]: # the number of active clients
            #for s_num in [10, 30, 50, 70, 100]:
            for s_num in [10, 30, 100]:
            #for s_num in [10]:
                # get the result directory where the result is stored
                res_dir = os.path.join(parent_dir, 'result_acc', 'parallel_fedcgds1.5', dat_name,  par_m + '#' + \
                        str(num_of_pars), selection + '#' + str(s_num), 'H#' + str(H_iter) + '_W#' + \
                        str(W_iter) + '_rho#' + str(rho) + '_nu#' + str(nu) + '_tol#' + str(tol)) 
