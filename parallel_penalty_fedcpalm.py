...
class Node(object):
    ...
    def __init__(self, data, data_num = 10000, rank = 4, init_W = None, init_H = None, \
            init_Z = None, init_rho = 1e-8, nu = 1e-4, mul = 0, max_val = 100, seed = 1):
    ...
    def update_HW_by_palm(self, max_iter):
        ''' This function performs the update of H_p by solving
            H_p, W_p = \arg\min_{H_p \in \mathcal{H}_p, W_p \in \mathcal{W}} F_p(W_p, H_p)
        ...
        '''
        N_p = self.H.shape[1]
        H_pre, W_pre = np.asmatrix(np.copy(self.H)), np.asmatrix(np.copy(self.W))
        for j in range(max_iter):
            ...
            self.W = np.maximum(0, np.minimum(self.max_val, W_pre - grad_W_pre / c))
            W_pre = np.asmatrix(np.copy(self.W))

        print ('H&W update converges #' + str(j))
        end_time = time.time()
...
def get_onmf_cost(data, n_factor, W, H, nu = 1e-4, mul = 0):
    ...
    '''
    (ha, hb) = H.shape
    res = LA.norm(data - W * H, 'fro')**2 / n_factor \
...
def fedcpalm(dat_gen = None, res_dir = 'res', num_of_cls = 10, num_of_locs = 100, seed = 1, \
        nu = 1e-4, rho = 1e-8, HW_max_iter = 1, DEBUG = False):
    ...
    while True:
        ...
        for node_id in range(num_of_locs):
            ...
            H_tmp, W_tmp = node.update_HW_by_palm(HW_max_iter)
            H_cur[:, local_idx_list[node_id]] = H_tmp
            W_cur += W_tmp * H_tmp.shape[1] / data_num
        
        if DEBUG:
            #onmf_cost = master_node.get_onmf_cost()
            sncp_cost = get_sncp_cost(data_mat, n_factor, W_cur, H_cur, nu, mul, rho)
            print ('SNCP_Iter(' + str(sncp_iter) + '), after update H&W, sncp_cost = ' + str(sncp_cost))
        ...
        res_manager.push_H_norm_ortho()  # store feasibility
        #res_manager.add_cost_value('onmf_cost', get_onmf_cost(data_mat, n_factor, W0_cur, H_cur, nu, 0)) # store obj val
        ...
        if W_change < 1e-4 or sncp_iter > 500: # converges
            print ('Converges------')
...
def main():
    ...
    for dat_name in ['snr-3']:
        ...
        for par_m in ['similarity']:
            ...
            #for HW_iter in [1, 5, 10, 100]:
            for HW_iter in [10]:
                # get the result directory where the result is stored
                res_dir = os.path.join(parent_dir, 'result', 'fedcpalm', dat_name,  par_m + '#' + \
                        str(num_of_pars), 'HW#' + str(HW_iter) + '_rho#' + str(rho) + '_nu#' + str(nu)) 
                ...
                #for seed_num in [1]:
                    ...
                    fedcpalm(dat_gen = data_gen, res_dir = seed_dir, num_of_cls = num_of_cls, \
                            num_of_locs = num_of_pars, seed = seed_num, nu = nu, rho = rho, \
                            HW_max_iter = HW_iter, DEBUG = True)
    print ('FedCPALM for clustering is done --------)')
